...
void Boot_animation()
{
	  	...
	  	OLED_Printf(0,17,OLED_8X16,"yaw:%+03.2f",yaw);
			...
			OLED_Printf(0,51,OLED_8X16,"P_Out:%+04d",Place_Out);
}
...
void Homepage_2()	
{ 
		OLED_Printf(0,0,OLED_8X16,"	A_out:%+04d",Angle_out);
		OLED_Printf(0,17,OLED_8X16,"f_err:%+03.1f",final_err);
		OLED_Printf(0,34,OLED_8X16,"P_Out:%+04d",Place_Out);
		OLED_Printf(0,51,OLED_8X16,"gz:%+05.2f",gyro_z);
		OLED_Update();
}
//第三页
void Homepage_3()	
...
void Homepage_4()	
{
			OLED_Printf(0,0,OLED_8X16,"	TF:%+03d",Turnback_Flag);
			OLED_Printf(0,17,OLED_8X16,"T:%+02d",Turnback_time);
			OLED_Printf(0,34,OLED_8X16,"FT:%+02d",Ten_Flag);
